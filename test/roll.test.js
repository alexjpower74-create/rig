...
function repoAt(path, { origin = null } = {}) {
  ...
  git(['add', '-A'], path)
  git(['commit', '-qm', 'initial'], path, OLD)
  if (origin) {
    git(['remote', 'add', 'origin', origin], path)
    git(['push', '-q', '-u', 'origin', 'main'], path)
  }
  return path
}
const commit = (repo, file, msg) => {
...
writeFileSync(
  join(home, '.claude', 'apps', 'millpond.md'),
  '---\nname: Millpond\nslug: millpond\ncode: ~/code/millpond\n---\n## What it is\nA fixture.\n',
)
const paths = {
...
await suite('rig roll', async (s) => {
  ...
  await s.check('a rollout brief parses: lists by tab, ordered procedure, commit subject, hard rules', {
    assert: async () => {
      ...
      if (r.procedure.length !== 4 || !r.procedure[0].startsWith('Record the test result'))
        throw new Error('procedure = ' + JSON.stringify(r.procedure))
    ...
    breaks: async () => {
      text = BRIEF.replace('## Lists', '## Lists of things')
      return () => {
        text = BRIEF
      }
    },
  ...
  await s.check('a slug in two lists is refused at parse time', {
    assert: async () => {
      try {
        parseRoll(BRIEF.replace('- lantern', '- lantern\n- ' + dup))
        return false
      } catch (e) {
        return /two lists/.test(e.message) && e.message.includes(dup)
      }
    },
    breaks: async () => {
      dup = 'ember'
      mkdirSync(join(home, 'Projects', 'ember'), { recursive: true })
      return () => {
        dup = 'kiln'
      }
    },
  ...
  await s.check('an explicit `slug = ~/path` under ## Repos wins over the registry and ~/Projects', {
    ...
    breaks: async () => {
      roll = parseRoll(BRIEF.replace('- kiln = ~/elsewhere/kiln', ''))
      return () => {
        roll = parseRoll(BRIEF)
      }
    },
  })
  await s.check('the registry’s code: field is used when there is no override', {
    ...
    breaks: async () => {
      const f = join(opts.registryDir, 'millpond.md')
  ...
  await s.check('~/Projects/<slug> is the last resort, and every path is absolute', {
    assert: async () => {
      const t = resolveRepos(roll, opts)
      return t.t1[0].path === paths.orchard && t.t2.every((r) => r.path.startsWith('/'))
    },
    breaks: async () => {
      const alt = { ...opts, projectsDir: join(home, 'nowhere') }
      const real = resolveRepos
      roll = parseRoll(BRIEF)
      const saved = opts.projectsDir
      opts.projectsDir = alt.projectsDir
      return () => {
        opts.projectsDir = saved
        void real
      }
    },
  })
  await s.check('a slug that resolves nowhere is an error before anything is launched, naming the slug', {
    assert: async () => {
      try {
        resolveRepos(parseRoll(BRIEF.replace('- lantern', '- lantern\n- ghost')), opts)
        return false
      } catch (e) {
        return /ghost/.test(e.message) && /Projects\/ghost/.test(e.message)
      }
    },
    breaks: async () => {
      mkdirSync(join(home, 'Projects', 'ghost'))
      return () => rmSync(join(home, 'Projects', 'ghost'), { recursive: true })
    },
  })
  ...
  mkdirSync(join(rollDir, 't1'), { recursive: true })
  mkdirSync(join(rollDir, 't2'))
  const tabs = resolveRepos(parseRoll(BRIEF), opts)
  const record = {
    brief: join(tmp, 'ROLL.md'),
    name: 'lint-sweep-2026-01-02',
    title: 'Lint sweep',
  ...
  const stateOf = (slug) => {
    const st = rollStatus(rollDir, { fetch: !fetchOff })
    for (const t of Object.values(st.tabs)) for (const r of t.repos) if (r.slug === slug) return r
    return null
  }
  ...
  await s.check('a clean repo with no commit since the roll started is untouched', {
    ...
    breaks: async () => {
      writeFileSync(join(paths.orchard, 'scratch.txt'), 'x')
      return () => rmSync(join(paths.orchard, 'scratch.txt'))
    },
  })
  ...
  await s.check('a dirty tree is in progress', {
    ...
    breaks: async () => {
      rmSync(join(paths.millpond, 'biome.json'))
      return () => writeFileSync(join(paths.millpond, 'biome.json'), '{}\n')
    },
  })
  ...
  await s.check('a commit since start that is not on origin is committed <short>, read after a fetch', {
    assert: async () => {
      ...
      return r.state === 'committed' && r.sha === kilnSha.slice(0, 7)
    },
    breaks: async () => {
      git(['push', '-q', 'origin', 'main'], paths.kiln)
      return () => {
        git(['push', '-q', '-f', 'origin', 'HEAD~1:main'], paths.kiln)
      }
    },
  })
  ...
  await s.check('a commit that origin has is pushed; the remote is asked, not remembered', {
    assert: async () => {
      const r = stateOf('beacon')
      return r.state === 'pushed' && r.sha === beaconSha.slice(0, 7)
    },
    // Move origin's main back one commit behind the rig's back: a stale tracking ref would still say pushed.
    breaks: async () => {
      const bare = join(tmp, 'remotes', 'beacon.git')
      const old = git(['rev-parse', 'main~1'], bare)
      git(['update-ref', 'refs/heads/main', old], bare)
      return () => git(['update-ref', 'refs/heads/main', beaconSha], bare)
    },
  })
  ...
  await s.check('pushed is re-read from a remote not named origin (fetch names the repo’s remote)', {
    assert: async () => {
      ...
      git(['update-ref', 'refs/heads/main', old], bare)
      try {
        return stateOf('beacon').state === 'committed'
      } finally {
        git(['update-ref', 'refs/heads/main', beaconSha], bare)
        git(['remote', 'rename', 'github', 'origin'], paths.beacon)
        git(['fetch', '-q', 'origin'], paths.beacon)
      }
    },
    // Read without the fetch: the stale tracking ref still says pushed.
    breaks: async () => {
      const f = join(rollDir, 'roll.json')
      void f
      fetchOff = true
      return () => {
        fetchOff = false
  ...
  await s.check('a commit with the roll subject but dated before the roll is not the roll’s (a re-run finds nothing)', {
    assert: async () => {
      ...
      try {
        return stateOf('kiln').state === 'untouched'
      } finally {
        git(['commit', '-q', '--amend', '--reset-author', '-m', 'chore: adopt the shared formatter'], paths.kiln)
      }
    },
    breaks: async () => {
      const f = join(rollDir, 'roll.json')
      const t = readFileSync(f, 'utf8')
      writeFileSync(f, JSON.stringify({ ...record, startedAt: '2019-01-01T00:00:00.000Z' }))
      return () => writeFileSync(f, t)
    },
  })
  await s.check('an unrelated commit during the roll is not the roll’s, and the table names the tab’s sha, not HEAD', {
    assert: async () => {
      ...
      try {
        ...
        writeFileSync(join(paths.orchard, 'x.txt'), 'x')
        git(['add', '-A'], paths.orchard)
        git(['commit', '-qm', 'owner: unrelated change'], paths.orchard)
        try {
          return stateOf('orchard').state === 'untouched'
        } finally {
          git(['reset', '-q', '--hard', 'HEAD~1'], paths.orchard)
        }
      } finally {
        if (git(['log', '-1', '--format=%s'], paths.kiln) === 'owner: unrelated change')
          git(['reset', '-q', '--hard', 'HEAD~1'], paths.kiln)
      }
    },
    breaks: async () => {
      const f = join(rollDir, 'roll.json')
      const t = readFileSync(f, 'utf8')
      writeFileSync(f, JSON.stringify({ ...record, commitSubject: null }))
      return () => writeFileSync(f, t)
    },
  })
  await s.check('a repo with no remote and a commit is local only', {
    assert: async () => {
      commit(paths.lantern, 'biome.json', 'chore: adopt the shared formatter')
      return stateOf('lantern').state === 'local only'
    },
    breaks: async () => {
      git(['remote', 'add', 'origin', join(tmp, 'remotes', 'beacon.git')], paths.lantern)
      return () => git(['remote', 'remove', 'origin'], paths.lantern)
    },
  })
  ...
  await s.check('an owner commit on top with an old author date does not hide the tab’s commit', {
    assert: async () => {
      const tab = kilnHead()
      writeFileSync(join(paths.kiln, 'pick.txt'), 'x')
      git(['add', '-A'], paths.kiln)
      git(['commit', '-qm', 'owner: cherry-picked from 2020'], paths.kiln, ownerDates)
      try {
        const r = stateOf('kiln')
        if (r.state !== 'committed' || r.sha !== tab) throw new Error(JSON.stringify(r) + ' tab=' + tab)
        return true
      } finally {
        git(['reset', '-q', '--hard', 'HEAD~1'], paths.kiln)
      }
    ...
    breaks: async () => {
      ownerDates = OLD
      return () => {
        ownerDates = { GIT_AUTHOR_DATE: OLD.GIT_AUTHOR_DATE }
      }
    },
  })
  await s.check('a remote not named origin is still a remote: an unpushed commit is committed, not local only', {
    assert: async () => {
      ...
      try {
        return stateOf('kiln').state === 'committed'
      } finally {
        git(['remote', 'rename', 'github', 'origin'], paths.kiln)
      }
    },
    breaks: async () => {
      const { remoteName } = await import('../src/roll.js')
      void remoteName
      git(['remote', 'remove', 'origin'], paths.kiln)
      return () => {
        git(['remote', 'add', 'origin', join(tmp, 'remotes', 'kiln.git')], paths.kiln)
        git(['fetch', '-q', 'origin'], paths.kiln)
      }
    },
  })
  ...
  await s.check('a slug the tab’s report marks SKIPPED is skipped', {
    ...
    breaks: async () => {
      const f = join(rollDir, 't1', 'REPORT.md')
      const t = readFileSync(f, 'utf8')
      writeFileSync(f, '# t1\n- orchard: done later\n')
      return () => writeFileSync(f, t)
    },
  ...
  await s.check('a repo the tab SKIPped for a dirty tree is skipped, not in progress (the brief’s own rule)', {
    assert: async () => {
      ...
      try {
        return stateOf('orchard').state === 'skipped'
      } finally {
        rmSync(join(paths.orchard, 'wip.txt'))
      }
    },
    breaks: async () => {
      const f = join(rollDir, 't1', 'REPORT.md')
  ...
  await s.check('a SKIPPED line over a repo that has a roll commit is a contradiction, not skipped', {
    ...
    breaks: async () => {
      git(['commit', '-q', '--amend', '--reset-author', '-m', 'chore: unrelated'], paths.kiln)
      return () => git(['commit', '-q', '--amend', '--reset-author', '-m', 'chore: adopt the shared formatter'], paths.kiln)
    },
  })
  ...
  const legacyLines = (text) => {
    ...
    for (const line of (text || '').split('\n')) {
      const m = line.match(/^\s*[-*]\s*`?([A-Za-z0-9._-]+)`?\s*:\s*(done|SKIPPED)\b/i)
      if (m) out.set(m[1], { state: m[2].toUpperCase() === 'SKIPPED' ? 'skipped' : 'done' })
    }
    return out
  }
  ...
  await s.check('a capitalised slug on a report line still names the lower-case slug', {
    assert: async () => {
      const l = linesFn('- Beacon: SKIPPED — busy\n- Lantern: done abc1234\n')
  ...
  await s.check('namedIn and skippedIn see the capitalised line too', {
    ...
    breaks: async () => {
      capLine = '- Beacon SKIPPED — busy\n'
      return () => {
        capLine = '- Beacon: SKIPPED — busy\n'
      }
    },
  })
  const legacyNamed = (text, slugs) =>
    new Set(
      slugs.filter((s) =>
        new RegExp('(^|[\\s`*-])' + s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?=[\\s`:*,.)]|$)', 'm').test(text || ''),
      ),
    )
  const legacySkipped = (text) => {
    const out = new Set()
    for (const line of (text || '').split('\n')) {
      const m =
        line.match(/^\s*[-*]?\s*`?([A-Za-z0-9._-]+)`?\s*:\s*SKIPPED\b/i) ||
        (/\bSKIPPED\b/.test(line) && line.match(/^\s*[-*]\s*`?([A-Za-z0-9._-]+)`?/))
      if (m) out.add(m[1])
    }
    return out
  }
  ...
  await s.check('report lines are read strictly: `- <slug>: done|SKIPPED` only; prose, headings and longer slugs do not count', {
    assert: async () => {
      ...
      if (lines.get('orchard').state !== 'done' || lines.get('orchard').sha !== 'def5678')
        throw new Error('orchard: ' + JSON.stringify(lines.get('orchard')))
      const sk = skippedFn(text)
      return sk.has('kiln') && !sk.has('orchard') && !sk.has('Note') && sk.size === 1
    },
    breaks: async () => {
      named = legacyNamed
      skippedFn = legacySkipped
      return () => {
        named = namedIn
        skippedFn = skippedIn
      }
  ...
  await s.check('rig roll finish fails while a repo is dirty or unpushed, and names each', {
    assert: async () => {
      ...
      return (
        !r.ok &&
        bad.some((n) => n.startsWith('millpond')) &&
        bad.some((n) => n.startsWith('kiln')) &&
        bad.some((n) => n.startsWith('t2 report'))
      )
    },
    breaks: async () => {
      // Make everything pass, then the assertion (which expects failure) must go red.
      rmSync(join(paths.millpond, 'biome.json'))
      commit(paths.millpond, 'biome.json', 'chore: adopt the shared formatter')
      git(['push', '-q', 'origin', 'main'], paths.millpond)
      git(['push', '-q', 'origin', 'main'], paths.kiln)
      writeFileSync(
        join(rollDir, 't1', 'REPORT.md'),
  ...
  await s.check('a passing finish writes ROLL-FINISH.md and jots "[workflow] roll <name>: N pushed, M skipped, K local" once', {
    assert: async () => {
      ...
      if (log.trim() !== '[workflow] roll lint-sweep-2026-01-02: 3 pushed, 1 skipped, 1 local')
        throw new Error('jot log: ' + JSON.stringify(log))
    ...
    breaks: async () => {
      const f = join(rollDir, 't2', 'REPORT.md')
      const t = readFileSync(f, 'utf8')
      writeFileSync(f, '# t2\n- beacon: done — formatted\n')
      rmSync(join(rollDir, 'ROLL-FINISH.md'), { force: true })
      return () => {
        writeFileSync(f, t)
        process.exitCode = 0
      }
  ...
  await s.check('a failing finish does not jot', {
    ...
    breaks: async () => {
      writeFileSync(process.env.JOT_LOG, 'stale\n')
  ...
  const realRunUp = (pane, extra = [], clean = true, envExtra = {}) => {
    ...
    if (clean) rmSync(rollsBase, { recursive: true, force: true })
  ...
  await s.check('rig roll up opens exactly one tab per list, each with --workspace <the rig’s own>, and none anywhere else', {
    assert: async () => {
      ...
      if (!rec.startedAt || rec.tabs.t1.length !== 3 || rec.tabs.t2[1].slug !== 'lantern' || rec.tabs.t1[2].path !== paths.kiln)
        throw new Error('roll.json: ' + JSON.stringify(rec))
  ...
  await s.check('the launch adds every other repo in the list with --add-dir, in list order', {
    ...
    breaks: async () => {
      runUp = (pane, extra = []) => realRunUp(pane, [...extra, '--launch', 'codex --model x'])
      return () => {
        runUp = realRunUp
      }
    },
  ...
  const legacyAddDirs = (cmd, repos) =>
    /^\s*claude(\s|$)/.test(cmd)
      ? repos
          .slice(1)
          .map((r) => ` --add-dir ${r.path}`)
          .join('')
      : ''
  let add = addDirs
  await s.check('--add-dir paths are shell-quoted, so a repo path with a space stays one argument', {
    assert: async () =>
      add('claude --effort low', [{ path: '/a/first' }, { path: '/a/mill house' }, { path: '/a/plain' }]) ===
      " --add-dir '/a/mill house' --add-dir /a/plain",
    breaks: async () => {
      add = legacyAddDirs
      return () => {
        add = addDirs
  ...
  await s.check('rig roll up refuses when a tab with one of its ids is already open, and creates nothing', {
    assert: async () => {
      let err = ''
      try {
        runUpWith({ HERDR_TABS: openTabs, HERDR_PANES: JSON.stringify([{ pane_id: 'w7:p5', tab_id: 'w7:t5' }]) })
      } catch (e) {
    ...
    breaks: async () => {
      openTabs = JSON.stringify([{ tab_id: 'w7:t5', label: 'c1' }])
      return () => {
        openTabs = JSON.stringify([{ tab_id: 'w7:t5', label: 't1' }])
      }
    },
  })
  await s.check('each tab’s brief carries its resolved repos, the procedure, the hard rules, the report path and the commit discipline', {
    assert: async () => {
      ...
      return (
        [
          '`kiln`  ' + paths.kiln,
          '1. Record the test result',
          '- Never commit over a dirty tree',
          join(rd, 't1', 'REPORT.md'),
          'Never run `wrap` or `sync`',
          'apps touch <slug>',
          'chore: adopt the shared formatter',
        ].every((x) => b.includes(x)) && !b.includes('beacon')
      )
    },
    breaks: async () => {
      const rd = join(rollsBase, readdirSync(rollsBase)[0])
      const f = join(rd, 't1', 'BRIEF.md')
      const t = readFileSync(f, 'utf8')
  ...
  await s.check('--dry-run and --help create nothing and launch nothing', {
    assert: async () => {
      ...
      const out = execFileSync(process.execPath, ['--input-type=module', '-e', script], {
        encoding: 'utf8',
        env: { ...process.env, HOME: home, RIG_ROLLS_DIR: rollsBase },
      })
  ...
  await s.check('rig roll down closes the roll’s tabs by id and leaves another crew’s tab alone', {
    assert: async () => {
      ...
      Object.assign(process.env, {
        ...
        HERDR_PANE_ID: 'w7:p1',
        HERDR_LOG: log,
        PATH: stubDir + ':' + savedPath,
        HERDR_TABS: JSON.stringify([
          { tab_id: 'w7:t1', label: 't1' },
          { tab_id: 'w7:t2', label: 't2' },
          { tab_id: 'w7:t3', label: 'c1' },
        ]),
      })
      try {
        down([rollDir], { terminal: 'herdr' })
      } finally {
        delete process.env.HERDR_ENV
        delete process.env.HERDR_PANE_ID
        process.env.PATH = savedPath
      }
      const closes = readFileSync(log, 'utf8')
        .split('\n')
        .filter((l) => l.startsWith('tab close'))
        .sort()
    ...
    breaks: async () => {
      const f = join(rollDir, 'roll.json')
      const t = readFileSync(f, 'utf8')
      writeFileSync(f, JSON.stringify({ ...record, tabs: { t1: record.tabs.t1 } }))
      return () => writeFileSync(f, t)
    },
  })
  ...
  await s.check('rig roll down refuses over a dirty repo unless --force', {
    assert: async () => {
      ...
      Object.assign(process.env, {
        ...
        HERDR_LOG: log,
        PATH: stubDir + ':' + savedPath,
        HERDR_TABS: JSON.stringify([{ tab_id: 'w7:t1', label: 't1' }]),
      })
      try {
        down(downArgs, { terminal: 'herdr' })
      } finally {
        delete process.env.HERDR_ENV
        delete process.env.HERDR_PANE_ID
        process.env.PATH = savedPath
        rmSync(join(paths.beacon, 'wip.txt'))
      }
