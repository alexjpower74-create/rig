...
import { mkdtempSync, writeFileSync, mkdirSync, existsSync, rmSync, realpathSync, readFileSync, chmodSync, symlinkSync } from 'node:fs'
...
mkdirSync(bin)
const binNoGh = join(tmp, 'bin-nogh')
mkdirSync(binNoGh)
...
writeFileSync(ghState.list, '[]')
writeFileSync(ghState.next, '11')
...
const setPath = (dir) => {
  process.env.PATH = dir === binNoGh ? `${dir}:${gitExec}` : `${dir}:${gitExec}:${saved.PATH}`
}
setPath(bin)
...
await suite('rig 3.0: up, guard, init', async (s0) => {
  ...
  const s = {
    check: (name, o) =>
      s0.check(name, {
        ...
        assert: async () => {
          try {
            return await o.assert()
          } catch (e) {
            if (process.env.RIG_TEST_DEBUG) console.error('      ' + e.message)
            throw e
          }
        },
      }),
  ...
  await s.check('two repos side by side get different default worktree bases, each named for its repo', {
    assert: async () => {
      const a = worktreeBase(alpha, loadConfig(alpha))
      const b = worktreeBase(beta, loadConfig(beta))
    ...
    breaks: async () => {
      ...
      for (const r of [alpha, beta]) {
        mkdirSync(join(r, '.rig'))
        writeFileSync(join(r, '.rig', 'config.json'), JSON.stringify({ worktreeDir: '../.shared-trees' }))
      }
      return () => {
        for (const r of [alpha, beta]) rmSync(join(r, '.rig'), { recursive: true })
      }
    },
  })
  ...
  await s.check('rig init writes the resolved worktreeDir and the slug, and gitignores .rig/ only', {
    ...
    breaks: async () => {
      ...
      mkdirSync(join(spaced, '.rig'))
      writeFileSync(join(spaced, '.rig', 'config.json'), JSON.stringify({ worktreeDir: '/elsewhere/trees' }))
  ...
  await s.check('a fresh repo with the guard hook installed makes its first commit', {
    ...
    breaks: async () => {
      ...
      const brokenRig = join(tmp, 'bin-broken')
      mkdirSync(brokenRig, { recursive: true })
      stub(brokenRig, 'rig', `case "$1" in guard) exit 1;; *) exec "${which('node')}" "${rigBin}" "$@";; esac\n`)
      const path = process.env.PATH
      process.env.PATH = `${brokenRig}:${path}`
      return () => {
        process.env.PATH = path
        if (before != null) writeFileSync(hookPath(), before)
      }
    },
  })
  ...
  mkdirSync(join(lead, '.rig'))
  writeFileSync(join(lead, '.rig', 'config.json'), JSON.stringify({ worktreeDir: '.worktrees' }))
  writeFileSync(join(lead, '.gitignore'), '.rig/\n.worktrees/\n')
  git(['add', '.gitignore'], lead)
  git(['commit', '-qm', 'ignore'], lead)
  git(['worktree', 'add', '-q', join(lead, '.worktrees', 'c1'), '-b', 'rig/c1'], lead)
  mkdirSync(join(lead, '.worktrees', 'c1', 'node_modules'))
  writeFileSync(join(lead, '.worktrees', 'c1', 'node_modules', 'x.js'), '1')
  writeFileSync(join(lead, '.worktrees', 'c1', 'README.md'), '# edited outside the slice\n')
  mkdirSync(join(lead, 'docs'))
  writeFileSync(join(lead, 'docs', 'review-c1.md'), '# review\n')
  ...
  await s.check('--staged in a slice worktree still refuses a file outside the slice', {
    assert: async () => {
      git(['reset', '-q'], c1wt)
      mkdirSync(join(c1wt, 'app'), { recursive: true })
      writeFileSync(join(c1wt, 'app', 'x.js'), '1\n')
  ...
  await s.check('rig up opens one issue per slice: create when none is open, reuse an open one, honour the plan’s Issue: line', {
    assert: async () => {
      rmSync(issuesPath(iss), { force: true })
      ...
      return (
        r.lines.length === 3 &&
        r.lines.some((l) => /c1\s+issue #11 \(opened\)/.test(l)) &&
        r.lines.some((l) => /c3\s+issue #3 \(reused/.test(l))
      )
    },
    breaks: async () => {
      git(['remote', 'remove', 'origin'], iss)
      return () => git(['remote', 'add', 'origin', 'https://example.invalid/o/r.git'], iss)
    },
  })
  ...
  await s.check('without gh, rig up says "no gh/remote: issues not opened" and carries on', {
    ...
    breaks: async () => {
      ...
      return () => {
        git(['remote', 'remove', 'origin'], alpha)
        setPath(bin)
      }
    },
  })
  ...
  await s.check('an agent’s brief carries "Your issue: #N <url>" and who closes it', {
    assert: async () => {
      ...
      const text = briefFor(plan, plan.agents[0], {
        branch: 'rig/c1',
        path: '/x',
        planPath: 'PLAN.md',
        reportPath: 'docs/build-report-c1.md',
        issue: issueCtx,
      })
      return text.includes(
        'Your issue: #11 https://example.invalid/o/r/issues/11 — the lead closes it at `rig finish`; put what is waiting on a person there',
      )
    },
    breaks: async () => {
      issueCtx = null
      return () => {
        issueCtx = { number: 11, url: 'https://example.invalid/o/r/issues/11' }
      }
    },
  })
  ...
  await s.check('rig init --slug --name creates the registry file with `apps new` once, and never when it exists', {
    assert: async () => {
      ...
      rmSync(logs.apps, { force: true })
      rmSync(join(reg, '.rig'), { recursive: true, force: true })
      ...
      if (JSON.parse(readFileSync(join(reg, '.rig', 'config.json'), 'utf8')).slug !== 'depot-draw')
        throw new Error('slug not written to config')
      betweenRuns()
    ...
    breaks: async () => {
      betweenRuns = () => rmSync(regFile, { force: true })
      return () => {
        betweenRuns = () => {}
      }
    },
  ...
  await s.check('rig init --no-registry skips apps; a bare rig init derives the slug from the repo dir', {
    assert: async () => {
      ...
      return (
        /registry: skipped/.test(r.stdout) &&
        slugFor('My Repo') === 'my-repo' &&
        JSON.parse(readFileSync(join(spaced, '.rig', 'config.json'), 'utf8')).slug === 'my-repo'
      )
    },
  ...
  let tpl = template
  await s.check(
    'the plan template carries the review rule, a Negative controls line and an Issue: line, and its placeholders parse as empty',
    {
      assert: async () => {
        const p = parsePlan(tpl)
        return (
          /rig review <id> --by <other>/.test(tpl) &&
          /^Negative controls:/m.test(tpl) &&
          /^Issue:/m.test(tpl) &&
          p.negativeCommand === null &&
          p.agents.every((a) => a.issue === null) &&
          !/Issue:/.test(p.agents[0].task)
        )
      },
      breaks: async () => {
        tpl = template.replace(/^Negative controls:.*\n/m, '')
        return () => {
          tpl = template
        }
      },
  ...
  await s.check('the AGENTS.md template says review before merge and a clean tree after npm test', {
    assert: async () =>
      /Review before merge:.*no slice merges without a review file on base/.test(agentsTpl) &&
      /After `npm test`, the tree is clean/.test(agentsTpl),
    breaks: async () => {
      agentsTpl = agentsTpl.replace(/^- \*\*Review before merge.*\n/m, '')
      return () => {
        agentsTpl = agentsOriginal
      }
    },
  })
  ...
  await s.check('review 1: the 2.0 literal "../.rig-worktrees" in a config reads as unset and gets the per-repo path', {
    assert: async () => {
      mkdirSync(join(alpha, '.rig'), { recursive: true })
      writeFileSync(join(alpha, '.rig', 'config.json'), JSON.stringify({ worktreeDir: legacyValue }))
      try {
        return loadConfig(alpha).worktreeDir === '../.rig-worktrees/alpha'
      } finally {
        rmSync(join(alpha, '.rig'), { recursive: true, force: true })
      }
    },
    breaks: async () => {
      legacyValue = '../.trees'
      return () => {
        legacyValue = '../.rig-worktrees'
      }
    },
  })
  ...
  await s.check('review 2: "Negative-control command for this repo: `cmd` …" and "Negative controls: `cmd` (note)" both yield cmd', {
    assert: async () =>
      ...
      negativeCommand(
        readFileSync(join(here, '..', 'PLAN.md'), 'utf8')
          .split('## Checks')[1]
          .split('## Rules')[0],
      ) === 'npm run demo',
    breaks: async () => {
      prose = 'Negative-control command for this repo: npm run demo must still print one VOID'
      return () => {
        prose = 'Negative-control command for this repo: `npm run demo` must still print exactly one VOID and one FAIL'
      }
    },
  })
  ...
  mkdirSync(join(det, 'app'))
  writeFileSync(join(det, 'app', 'x.js'), '1\n')
  ...
  await s.check('review 3: on a detached HEAD, --staged --agent c1 still refuses a file outside c1; headState tells detached from unborn', {
    assert: async () => {
      ...
      git(['reset', '-q'], det)
      if (bare.status !== 0 || !/detached HEAD/.test(bare.stdout))
        throw new Error(`without --agent: ${bare.status} ${bare.stdout} ${bare.stderr}`)
  ...
  const busy = (openTitle) =>
    JSON.stringify([
      ...
      ...Array.from({ length: 31 }, (_, i) => ({
        ...
        title: 'c1 old build',
        state: 'CLOSED',
        url: `https://example.invalid/o/r/issues/${100 + i}`,
      })),
    ])
  ...
  await s.check('review 4: with 31 closed "c1 …" issues on the first page, the one open one is still reused (no duplicate created)', {
    assert: async () => {
      ...
      const page = JSON.parse(
        execFileSync('gh', ['issue', 'list', '--state', 'all', '--search', 'c1 in:title', '--json', 'x'], { encoding: 'utf8', env }),
      )
      ...
      try {
        ...
        delete process.env.GH_FULL
      }
    },
    breaks: async () => {
      openTitle = 'c1x not this slice'
      return () => {
        openTitle = 'c1 The entry page'
      }
    },
  })
  ...
  await s.check('review B: "Negative controls: <e.g. `npm run demo`>" is a placeholder and yields null', {
    ...
    breaks: async () => {
      placeholder = 'Negative controls: e.g. `npm run demo`'
      return () => {
        placeholder = 'Negative controls: <e.g. `npm run demo`>'
      }
    },
  ...
  await s.check('review C: detached HEAD inside <worktreeDir>/c1, `rig guard --staged` with no --agent still refuses a stray file', {
    assert: async () => {
      ...
      mkdirSync(join(c1wt, 'app'), { recursive: true })
      writeFileSync(join(c1wt, 'app', 'x.js'), '2\n')
  ...
  await s.check('review D: a "must not happen" rule wrapped onto an indented line reaches the brief whole', {
    assert: async () => {
      ...
      const brief = briefFor(plan, plan.agents[0], {
        branch: 'rig/c1',
        path: '/x',
        planPath: 'PLAN.md',
        reportPath: 'docs/build-report-c1.md',
      ...
      return (
        plan.mustNotRules[0] === 'Never run wrap or sync unless --wrap was passed.' &&
        brief.includes('- Never run wrap or sync unless --wrap was passed.')
      )
    },
    breaks: async () => {
      indent = ''
  ...
  process.env.HERDR_PANE_ID = 'w7:p0'
  ...
  await s.check('herdr: attachHint says "this workspace" and a new slice tab is created with --workspace <our workspace>', {
    assert: async () => {
      ...
      if (herdrCalls().some((l) => /^workspace create/.test(l) || /^pane split/.test(l)))
        throw new Error('opened a workspace or split a pane')
      return true
    },
    breaks: async () => {
      process.env.HERDR_PANE_ID = 'w8:p0'
      return () => {
        process.env.HERDR_PANE_ID = 'w7:p0'
      }
    },
