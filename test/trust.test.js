...
async function withHome (setup, fn) {
  ...
  try { setup(home); const { preTrust } = await import('../src/trust.js?' + Date.now()); return await fn(preTrust, home) }
...
test('preTrust marks the worktree trusted in ~/.claude.json', async () => {
  await withHome(h => fs.writeFileSync(path.join(h, '.claude.json'), JSON.stringify({ projects: {} })), (preTrust, h) => {
