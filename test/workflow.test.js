...
await suite('rig 2.0, workflow', async (s) => {
  ...
  await s.check('every agent’s brief carries the plan’s “must not happen” rules word for word', {
    assert: async () => {
      ...
      return (
        text.includes("- Never claim a refund amount it can't back up.") &&
        text.includes('- Nothing is sent to a customer without the owner.')
      )
    },
    breaks: async () => {
      planText = FILLED.replace(/## What must not happen[\s\S]*?## Where/, '## Where')
      return () => {
        planText = FILLED
      }
    },
  ...
  await s.check('a ### subheading in the brief is not a slice (slices live under ## Agents)', {
    assert: async () => {
      const p = join(tmp, 'scoped-plan.md')
      writeFileSync(p, scopedText)
      try {
        return (
          loadPlan(p)
            .agents.map((a) => a.id)
    ...
    breaks: async () => {
      scopedText = FILLED.replace('## Agents\n', '')
      return () => {
        scopedText = FILLED
      }
    },
  ...
  await s.check('a slice id that is a path (like ..) is refused before anything uses it', {
    assert: async () => {
      const p = join(tmp, 'id-plan.md')
      writeFileSync(p, FILLED.replace('### c1 — Entry page', `### ${badId} — Entry page`))
      try {
        loadPlan(p)
        return false
      } catch (e) {
        return /not a plain name/.test(e.message)
      }
    },
    breaks: async () => {
      badId = 'c9'
  ...
  git(['add', '-A'], repo)
  git(['commit', '-qm', 'c1: entry page'], repo)
  ...
  await s.check('a review brief points a fresh reviewer at exactly that slice’s diff and the hard rules', {
    assert: async () => {
      ...
      return (
        r.files.includes('app/entry.js') &&
        r.text.includes('git diff main...rig/c1') &&
        r.text.includes('Never claim a refund amount') &&
        /Read only/.test(r.text)
      )
    },
    breaks: async () => {
      reviewBase = 'rig/c1'
  ...
  git(['add', '-A'], repo)
  git(['commit', '-qm', 'review c1'], repo)
  const head = git(['rev-parse', 'HEAD'], repo)
  const writeLog = (entries) => {
    rmSync(qaLogPath(repo), { force: true })
    for (const e of entries) recordQa(repo, e)
  }
  ...
  await s.check('finish passes when merged, clean, and QA exited 0 on this exact commit', {
    ...
    breaks: async () => {
      recordQa(repo, { sha: head, exit: 1, cmd: 'npm test' })
      return () => writeLog([{ sha: head, exit: 0, cmd: 'npm test' }])
    },
  })
  ...
  await s.check('a green QA run on a different commit does not count', {
    assert: async () => {
      const r = finishChecks(repo, cfg, loadPlan(join(repo, 'PLAN.md')), 'main')
      return r.failed && r.checks.some((c) => /QA green/.test(c.name) && c.state === 'fail')
    },
    breaks: async () => {
      recordQa(repo, { sha: head, exit: 0, cmd: 'npm test' })
      return () => writeLog([{ sha: oldSha, exit: 0, cmd: 'npm test' }])
    },
  })
  ...
  await s.check('a slice that was never built (no branch, no report) fails the gate instead of reading “merged”', {
    assert: async () => {
      const r = finishChecks(repo2, cfg, loadPlan(join(repo2, 'PLAN.md')), 'main')
      return r.checks.some((c) => c.name === 'c2 merged' && c.state === 'fail')
    },
    breaks: async () => {
      writeFileSync(join(repo2, 'docs', 'build-report-c2.md'), '# c2\n')
      git(['add', '-A'], repo2)
      git(['commit', '-qm', 'c2 report'], repo2)
      return () => {
        git(['reset', '-q', '--hard', 'HEAD~1'], repo2)
      }
    },
  })
  ...
  mkdirSync(join(repo3, 'app'))
  writeFileSync(join(repo3, 'app', 'entry.js'), 'export const entry = 1\n')
  git(['add', '-A'], repo3)
  git(['commit', '-qm', 'c1 part 1'], repo3)
  writeFileSync(join(repo3, 'app', 'entry.js'), 'export const entry = 2\n')
  git(['add', '-A'], repo3)
  git(['commit', '-qm', 'c1 part 2'], repo3)
  ...
  await s.check('a squash-merged slice counts as merged', {
    assert: async () => {
      const r = finishChecks(repo3, cfg, loadPlan(join(repo3, 'PLAN.md')), 'main')
      return r.checks.some((c) => c.name === 'c1 merged' && c.state === 'ok')
    },
    breaks: async () => {
      execFileSync('git', ['checkout', '-q', 'rig/c1'], { cwd: repo3 })
      writeFileSync(join(repo3, 'app', 'more.js'), 'unmerged\n')
      git(['add', '-A'], repo3)
      git(['commit', '-qm', 'c1 part 3, never merged'], repo3)
      execFileSync('git', ['checkout', '-q', 'main'], { cwd: repo3 })
      return () => {
        git(['branch', '-f', 'rig/c1', 'rig/c1~1'], repo3)
      }
    },
  })
