...
await suite('rig 2.0, crew fixes', async (s) => {
  ...
  const legacyTabs = (tabs, _ids, home) => tabs.filter((t) => t.label && t.tab_id !== home && /^[a-z]\d+$/i.test(t.label))
  const TABS = [
    { tab_id: 'w:t1', label: 'Conductor' },
    { tab_id: 'w:t2', label: 'jr-lead' },
    { tab_id: 'w:t3', label: 'jr1' },
    { tab_id: 'w:t4', label: 'jr2' },
    { tab_id: 'w:t5', label: 'c1' },
    { tab_id: 'w:t6', label: 'nb1' },
  ]
  ...
  await s.check('slice tabs are exactly the plan’s ids: two-letter ids found, other crews’ tabs untouched', {
    assert: async () => {
      const got = pick(TABS, ['jr1', 'jr2'], 'w:t2')
        .map((t) => t.label)
        .join(',')
  ...
  await s.check('the tab the rig runs in is never a slice tab, even when labelled like one', {
    ...
    breaks: async () => {
      pickHome = ignoresHome
      return () => {
        pickHome = selectSliceTabs
      }
    },
  })
  ...
  const legacyRef = (args, fallback) => {
    const a = (n) => {
      const i = args.indexOf(n)
      return i >= 0 ? args[i + 1] : null
    }
    return a('--ref') || a('--branch') || fallback
  }
  let ref = resolveRef
  ...
  const legacyDetached = (root, cfg, id, r) => {
    ...
    try {
      git(['checkout', '--detach', r], path)
    } catch {
      git(['worktree', 'add', '--detach', path, r], root)
    }
    return { path }
  }
  ...
  await s.check('re-pinning the QA worktree to HEAD after HEAD~1 lands on the main checkout’s HEAD', {
    assert: async () => {
      try {
        execFileSync('git', ['worktree', 'remove', '--force', join(qrepo, '.worktrees', 'qa')], { cwd: qrepo, stdio: 'ignore' })
      } catch {}
  ...
  await s.check('a run killed by a signal is not exit 0', {
    assert: async () =>
      exitOf(null, 'SIGTERM') === 143 && exitOf(null, 'SIGKILL') === 137 && exitOf(0, null) === 0 && exitOf(2, null) === 2,
    breaks: async () => {
      exitOf = legacyExit
      return () => {
        exitOf = exitCodeFor
  ...
  const fresh = (id, at) => {
    try {
      execFileSync('git', ['worktree', 'remove', '--force', join(repo, '.worktrees', id)], { cwd: repo, stdio: 'ignore' })
    } catch {}
  ...
  await s.check('the move rule: only a branch with nothing unmerged and base moved on', {
    assert: async () =>
      decide({ ahead: 0, behind: 3 }) === true &&
      decide({ ahead: 1, behind: 3 }) === false &&
      decide({ ahead: 0, behind: 0 }) === false &&
      decide({ ahead: 0, behind: 3 }, { keepBranches: true }) === false,
    breaks: async () => {
      decide = legacyShouldMove
      return () => {
        decide = shouldMoveLeftover
      }
    },
  })
  ...
  await s.check('“inside a worktree” means the directory or below it — not a sibling that shares its prefix', {
    assert: async () =>
      within('/w/wt-a/src', '/w/wt-a') &&
      within('/w/wt-a', '/w/wt-a') &&
      within('/w/wt-a/x (deleted)', '/w/wt-a') &&
      !within('/w/wt-ab', '/w/wt-a') &&
      !within('/w', '/w/wt-a'),
    breaks: async () => {
      within = legacyInside
      return () => {
  ...
  const dirA = join(tmp, 'wt-a')
  const dirAB = join(tmp, 'wt-ab')
  const linkA = join(tmp, 'link-to-wt-a')
  mkdirSync(dirA)
  mkdirSync(dirAB)
  ...
  const main = join(tmp, 'deps-main')
  const wtLink = join(tmp, 'deps-wt-link')
  const wtReal = join(tmp, 'deps-wt-real')
  mkdirSync(join(main, 'node_modules'), { recursive: true })
  writeFileSync(join(main, 'package.json'), '{}')
  mkdirSync(wtLink)
  symlinkSync(join(main, 'node_modules'), join(wtLink, 'node_modules'))
