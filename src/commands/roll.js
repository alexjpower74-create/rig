...
export default async function roll(args) {
  ...
  switch (verb) {
    case 'up':
      return up(rest, cfg)
    case 'status':
      return status(rest, cfg)
    case 'finish':
      return finish(rest, cfg)
    case 'down':
      return down(rest, cfg)
    default:
      throw new Error(`rig roll needs a verb: up | status | finish | down\n\n${USAGE}`)
  }
}
...
export function up(args, cfg) {
  ...
  if (!noLaunch) {
    ...
    if (term.available()) {
      ...
      if (open.length)
        throw new Error(
          `tab(s) already open in this workspace with these ids: ${open.join(', ')}. Give this roll's lists their own ids (they are the tab labels), or \`rig roll down\` the roll that owns them.`,
        )
    }
  }
  ...
  for (const id of ids) {
    ...
    if (dry) {
      console.log(`  ${id}  would open a tab at ${tabs[id][0].path} for: ${line}`)
      continue
    }
  ...
  const record = {
    ...
    title: roll.title,
    startedAt,
    commitSubject: roll.commitSubject || null,
...
export function addDirs(launchCmd, repos) {
  ...
  return repos
    .slice(1)
    .map((r) => ` --add-dir ${shellQuote(r.path)}`)
    .join('')
}
...
export function pickRoll(args) {
  ...
  const rolls = readdirSync(base)
    .map((d) => join(base, d))
    .filter((d) => existsSync(join(d, 'roll.json')))
...
export function status(args, cfg) {
  ...
  for (const id of ids) {
    ...
    for (const r of t.repos) {
      ...
      console.log(
        `   ${r.slug.padEnd(24)} ${paint(r.state)}${r.sha ? ' ' + r.sha : ''}${r.state === 'committed' ? DIM(' (not on ' + r.remote + ')') : ''}`,
      )
    }
    console.log()
  }
  ...
  if (inProgress) {
    console.log(YEL(`${inProgress} repo(s) in progress.`))
    process.exitCode = 1
  } else console.log(GRN('no repo mid-edit.'))
...
export function finishChecks(rollDir, opts = {}) {
  ...
  for (const [id, t] of Object.entries(tabs)) {
    if (!t.reportExists) checks.push({ name: `${id} report`, ok: false, detail: `missing: ${t.reportPath}` })
      ...
      checks.push({
        name: `${id} report names every repo`,
        ok: !unnamed.length,
        detail: unnamed.length ? `not in the report: ${unnamed.join(', ')}` : `${t.repos.length} repo(s) accounted for`,
      })
    }
    for (const r of t.repos) {
      ...
      const why =
        r.state === 'contradiction'
          ? `report says SKIPPED but ${r.sha} carries the roll's commit subject; one of them is wrong`
          : r.state === 'in progress'
            ? 'uncommitted changes'
            : r.state === 'committed'
              ? `${r.sha} is not on ${r.remote}`
              : r.state === 'untouched'
                ? 'no commit since the roll started and not SKIPPED in the report'
                : r.state === 'missing'
                  ? 'folder is gone'
                  : `${r.state}${r.sha ? ' ' + r.sha : ''}`
...
export function finish(args, _cfg) {
  const rollDir = pickRoll(args)
  ...
  const jotNote = jot.ok
    ? `jot: ${jotLine}`
    : `jot not run (${/ENOENT/.test(jot.err) ? 'not on PATH' : jot.err.split('\n')[0]}): ${jotLine}`
  writeFileSync(
    join(rollDir, 'ROLL-FINISH.md'),
    `# ${record.title} — finished ${new Date().toISOString()}
...
export function rollTable(rows) {
  ...
  return [
    line('slug', 'tab', 'state', 'sha'),
    line('-'.repeat(w[0]), '-'.repeat(w[1]), '-'.repeat(w[2]), '-------'),
    ...rows.map((r) => line(r.slug, r.tab, r.state, r.sha || '-')),
  ].join('\n')
}
...
export function down(args, cfg) {
  ...
  const ids = Object.keys(tabs)
  const dirty = Object.values(tabs)
    .flatMap((t) => t.repos)
  ...
  const term = terminal(cfg)
  const session = sessionFor(record.name)
  if (!term.available() || !term.sessionExists(session)) {
    console.log(`${term.name} not available or no session; nothing to close`)
    return
  }
