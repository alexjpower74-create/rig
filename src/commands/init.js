...
export function ensureRegistry(slug, name, opts = {}) {
  ...
  said.path = file
  if (opts.skip) {
    said.push('registry: skipped (--no-registry)')
    return said
  }
  if (existsSync(file)) {
    said.push(`registry: ${file} exists; left alone`)
    return said
  }
  if (!onPath('apps')) {
    said.push('registry: no `apps` on PATH; create the registry file yourself when you can')
    return said
  }
  const r = tryRun('apps', ['new', slug, name])
  if (!r.ok) {
    said.push(`registry: apps new ${slug} failed: ${(r.err || r.out).split('\n')[0]}`)
    return said
  }
  said.ran = true
