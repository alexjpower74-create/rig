...
// `qa-2`, `qa-3`…; "free" means no live pid in `<path>/.rig/qa.lock`. The lock is written before
// the checkout and removed after the run, on SIGINT and SIGTERM too.

export const qaLockPath = path => join(path, '.rig', 'qa.lock')
export const QA_LOCK_KEEP = '.rig/qa.lock'

/** { live, pid } for the lock at a QA worktree path. A pid that is not running is not live. */
...
export function acquireQaWorktree (root, cfg, ref, opts = {}) {
  ...
  for (let n = 1; n <= 64; n++) {
    ...
    // A worktree that is not there yet is made first: git refuses to add one over a non-empty
    // directory, and the lock needs a directory to live in.
    if (!existing) ensureDetachedWorktree(root, cfg, id, ref, opts)
    mkdirSync(join(path, '.rig'), { recursive: true })
    try { writeFileSync(qaLockPath(path), String(process.pid) + '\n', { flag: 'wx' }) } catch (e) {
      if (e.code === 'EEXIST') { notes.push(`${id} was taken while we looked`); continue }
      throw e
    }
    const wt = ensureDetachedWorktree(root, cfg, id, ref, { ...opts, keep: [QA_LOCK_KEEP, ...(opts.keep || [])] })
    const release = () => {
